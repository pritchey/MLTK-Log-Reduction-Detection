...
class simPlugin(object):
	...
	def print_help(self):
		...
		"""
		text = "\tName: ml_scikit_tsvd_euclideandistance"
		text += "\n\t\tThis machine learning plugin uses scikit-learn's truncated SVD algorithm and calculates euclidean distance.\n"
		text += "\n\t\tOptional Parameters:"
		text += "\n\t\t\ttsvdeuclideandistance_skip_normalization:  Do NOT perform normalization (scaling) of data, skip this step."
	...
	def machine_learning(self, df, plugin_options):
		...
		the euclidean distance from the centroid to each data point in the results.
		...
		print("\tMachine learning algorithm:  scikit-learn TSVD and calculates Euclidean Distance to centroids.")
		...
		if "tsvdeuclideandistance_skip_normalization" not in plugin_options:
		...
		centroid = np.mean(US, axis = 0)
		print("\ncentroid = ", centroid)
		print("\tFinished calculating centroid...")

		print("\tCalculating Euclidean distance...")
		distance = []
		for i in US:
			distance.append(np.linalg.norm(i - centroid))
		print(US[0])
		...
		return {"US": US, "S": S, "VT": VT, "T": T, "graph": "US", "centroid": centroid, "distance": distance}