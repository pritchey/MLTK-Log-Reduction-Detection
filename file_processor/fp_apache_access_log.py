...
class simPlugin(object):
	...
	def print_help(self):
		...
		"""
		return "\tName: fp_apache_access_log\n\t\tThis file parser processes Apache access log files.\n"
	...
	def process_file(self, fileName):
		...
		for entry in parser.parse_lines(fp):
			...
			entry.request_time.minute, entry.request_time.second, entry.remote_host, entry.request_line, entry.bytes_sent, category,
			entry.headers_in["Referer"], entry.headers_in["User-Agent"]]
			)
		...
		self.df = DataFrame(self.raw_data, columns=["year", "month", "day", "hour", "minute", "second", "remote_host", "request", "bytes_sent", "status", "referer", "user-agent"])
