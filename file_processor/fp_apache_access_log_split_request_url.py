...
class simPlugin(object):
	...
	def process_line(self, entry):
		...
		entry.headers_in["Referer"], entry.headers_in["User-Agent"], command, parameters, version]
	...
	def process_file(self, fileName):
		...
		# which can vary.
		self.build_Column_Names(["year", "month", "day", "hour", "minute", "second", "remote_host", "bytes_sent", "status", "referer", "user-agent", "command", "parameters", "version"])
		...
		self.df = pd.concat([self.df, self.url_splits], axis=1, ignore_index=True)
