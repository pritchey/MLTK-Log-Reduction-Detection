...
class simPlugin(object):
	...
	def print_help(self):
		...
		"""
		return "\tName: fp_linux_mail_log\n\t\tThis file parser processes Linux mail daemon log files (/var/log/maillog).\n"
	...
	def process_file(self, fileName):
		...
		for entry in fp:
			...
			deduplicated = entry.replace("  ", " ").replace("  ", " ")
			...
			# Due to variable log formatting, not all log lines will have a message ID.  Detect this and
			# cause later automatic appending to automatically adjust.
			if ":" not in extracted[5]:
				row.append("No_Message_ID")
				append_from = 5
			elif len(extracted[5]) <= 8:
				row.append("No_Message_ID")
				append_from = 5
			else:
				row.append((extracted[5]).replace(":", ""))
				append_from = 6

			# Now detect if we have an hostname (a.b.c format) or something else in the next feature.
			if ((extracted[append_from]).count(".") == 2) and ((extracted[append_from]).count("from=") == 0) and ((extracted[append_from]).count("to=") == 0):
				# We have a hostname, append it...
				row.append(extracted[append_from])
				append_from += 1
			else:
				# Detect hostname in the format of a.b.c.d
				if ((extracted[append_from]).count(".") == 3) and ((extracted[append_from]).count("from=") == 0) and ((extracted[append_from]).count("to=") == 0):
					foo = extracted[append_from].split(".")[1]
					try:
						int(foo)
						# If the above line doesn't cause an exception, it's an IP address.
						row.append("No_Hostname")
					except:
						# Typecasting failed, it means we have a string from a hostname.
						row.append(extracted[append_from])
						append_from += 1
				else:
					row.append("No_Hostname")
			...
			if ((extracted[append_from]).count(".") == 3) and ((extracted[append_from]).count("from=") == 0):
				# We have an IP address.
				row.append((extracted[append_from]).replace("]", "").replace("[", ""))
				append_from += 1
			else:
				row.append("No_IP_Address")

			# Due to splitting the log line, the remainder past the daemon will vary.  Dynamically append.
			for z in extracted[append_from:]:
				row.append(z)
		...
		print("\tSample reduced entry: %s" % self.df.iloc[0])
